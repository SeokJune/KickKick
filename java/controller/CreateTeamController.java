	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {	
			if(cmd.equals("/hometown.team")) {
				...
				request.getRequestDispatcher("/team/teamcreateForm.jsp").forward(request, response);
			}
				...
				String hometown_name = request.getParameter("hometown_name");
				String outline = request.getParameter("outline");
				String content = request.getParameter("content");
				CreateTeamDTO dto = new CreateTeamDTO(team_name, captain_name, captain_phone, hometown_name, outline, content);
				CreateTeamDAO dao = CreateTeamDAO.getInstance();
				dao.insertTeam(dto);
