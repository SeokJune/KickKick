	...
	public List<CompetitionListDTO> selectlist() throws Exception {
		...
		try (Connection con = this.getConnection();
			...
			while (rs.next()) {
				...
				CompetitionListDTO dto = new CompetitionListDTO(team_name, member_name, member_phone, logo_path, logo,
						latirude, longitude, competition_date, status_code, status_name, kind_name, code);
				list.add(dto);
			}
			return list;
		}
...
//		}
	// 날짜로 게임 경기 리스트 검색
	public List<CompetitionDTO> match_game_list(String date) throws Exception {
		String sql = "select * from competition_view \r\n"
				+ "where to_char(COMPETITION_DATE,'yy/mm/dd') = ? and status_code=1301";
		try (Connection con = this.getConnection(); PreparedStatement pstat = con.prepareStatement(sql);) {
			pstat.setString(1, date);
			try (ResultSet rs = pstat.executeQuery();) {
				...
				while (rs.next()) {
					...
					CompetitionDTO dto = new CompetitionDTO(0, 0, null, 0, 0, 0, competition_date, 0,
							registration_team_name, 0, 0, application_team_name, 0, 0, null);
					list.add(dto);
				}
				return list;
			}
